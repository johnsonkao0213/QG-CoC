...
# domain_tasks["High-level-obj-semantic"]="face_retrieval functional_correspondence_blink functional_correspondence_funk_point handwritten_retrieval image2image_retrieval person_reid semantic_correspondence_blink"
# domain_tasks["High-level-obj-semantic"]="semantic_correspondence_misc210k sketch2image_retrieval spot_the_diff spot_the_similarity text2image_retrieval vehicle_retrieval visual_correspondence_blink"
# domain_tasks["High-level-obj-semantic"]="visual_correspondence_scannet visually_grounded_reasoning_marvl visually_grounded_reasoning_nlvr2"
...
for DOMAIN in "${!domain_tasks[@]}"; do
    window_name="${MODEL}_${DATASET}_${DOMAIN}_${METHOD}"
    ...
    for task in "${task_list[@]}"; do
        ...
        tmux new-window -t $window_name:$index
        tmux send-keys -t $window_name:$index "conda activate lmms-eval" Enter
        tmux send-keys -t $window_name:$index "python I2T_inference.py --dataset $DATASET --domain $DOMAIN --task $task --engine $MODEL --prompting_method $METHOD" Enter
        ((index++))
    done
done
